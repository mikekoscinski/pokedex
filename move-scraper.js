	...
	moveTableElements.forEach(table => {
		...
		moveEntries.forEach(entry => {
			...
			const level_obtained = (tableIndex === 0) ?
				...
				''
				// We only need 'level_obtained' property for moves obtained via level-up
			;
			...
			// TODO: Eventually need to interpolate 'pokemon_name' from the 'name' parameter from master runScraper(pokemonName) function. Currently, hardcoded to fit our test example.
			const newMoveToAdd = `{ pokemon_name: 'Nidoking', move_id: ${move_id}, method_obtained: ${method_obtained}${level_obtained}}`;
			allMovesForOnePokemon.push(newMoveToAdd);
		});
	});
};
storeMoves();
